...
def input_pipeline(filename,param):
    ...
    shuffle(minibatched_scans)     #Shuffle to randomize for input into the model
...
def BuildRegrTbl(XP):
    ...
    p48N = pd.read_csv(PATH1 + "/predict48H.csv",delimiter="|").set_index('pzvID')
...
def viewCombiner2(Threat_Rgn)  :
    import pickle    
    global z1,NewB_intermediates,NewB_Pr,training_intermediates_ML
    ...
    NewB_intermediates = BuildRegrTbl(NewBPredictions)[:,0:-1] #omit last colum which is blank
    pzvidE = NewB_intermediates[:,0] #this is pzvid

    NewB_Pr = np.minimum(1,np.maximum(logr.predict_proba(NewB_intermediates[:,2:9]),0))


    pzInvty = pd.read_csv(PATH1 + "/pz_invty4.csv",delimiter="|").set_index('PZidq')
    
    #Need the PZs of all NewB
    #Will need this for minimbatch to add thse passegner to PZinvty4
    NewB= pzInvty[pzInvty["Probability"]=="NewB"]["PZ"]  #
    ...
    with open(qfile,"w") as outFile:
        ...
        for k in range(len(pzvidE)):
          v1=NewB[pzvidE[k]].replace("_z","_Zone")
          outFile.write("\n%s,%0.4f" % (v1,NewB_Pr[k][1] ) )
...
prefix= '/Hidden_' #        prefix= '/NewBatch_'

#these are zones not zm1
for Threat_Rgn in rgnlist:
    ...
    ## Building deep neural network
        ...
        model = new_model
        print("\nModelLoaded.Predicting...")
    
        #adapt this to predict the new ones
   

     
        NewB_pzid, dummy, NewB_im = input_pipeline(PPDF + prefix + Threat_Rgn+".npy",2) #hatch-lb is zeros
        NewB_im = NewB_im.reshape(-1, Dm1, Dm1, 1)
        NewBPredictions=model.predict({'imAges': NewB_im})
        NewBPredictions[:,0]=NewB_pzid #overwrite column zero
        np.savetxt(PREDICT_PA2 + "/NewB_"+ Threat_Rgn +"_.csv", NewBPredictions, delimiter="|")
        print("saved compsite NewB single image pred file with shape ",NewBPredictions.shape)              
