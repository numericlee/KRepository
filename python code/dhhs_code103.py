...
def zoneslicelist1(zone,view,filetype):
    ...
    if zon==4 and viewq in [16.24, 40,48,48]: #revised
        zsl=nullZSL
...
def RunMetrix(NewPath,p2LST,filnam,dump1,param,XYZ):
    ...
    file_lim = 300000 #higher for stage2 500 in trail mode
    ...
    with open(dump1,"w") as DhumpFile: #vary for hidden run?
        ...
        PZVfiles =   pd.read_csv(PATH1 + r"/file_inventory/pzv_invty_20171212.csv",delimiter = "|")
        ...
        for ktr,pzv in enumerate(PZVlist[0:file_lim]):
            ...
            if pz2idx >=0: #for part one (stage I) filters out stage II names
                ...
                with open( NewPath  +  pzv + ".csv",'r') as inp_f:
...
def pp_saver1(qFile2,qFile1,label,pz_id,pssgr):
    global savaBatch,savNewB

    try:
        image = np.genfromtxt(qFile2,delimiter='|')
    except:
         try:
             image = np.genfromtxt(qFile1,delimiter='|')

         except:
            print("error_missing",qFile2)
            #tom=input("dick and harry")
            return
    ...
    if label !=[3,-2]:
        ...
        savNewB.append([pz_id,image ]) #no need to flio  
...
def MiniBatch(region):
    global savNewB,savaBatch,pzSubset,pzMinibatch #uncommented
    ...
    label_dict = {"1": [0,1], "0": [1,0],"Hidden":[2,-1],"NewB":[3,-2]} #the Hidden Labels should be gone
    ...
    pzInvty = pd.read_csv("pz_invtyStage1_2_Combo.csv",delimiter="|") 
    ...
    #in stage 2, we compute the size off the total combined length and we dont shuffle.
    #this means the first 47% are all batched one way and the last 53% all go to NB
    #however, the code as written will continue to write training minibatches for the latter 53%; but they will be empty
    #we could fix this but it instead we simply ignore the later minibatches -which are eff emppty
    #it also means the NB files are extremely large
    ...
    for batch_num  in range(batches): #batches number from zero
        ...
        for index, row in pzMinibatch.iterrows():
            ...
            label=label_dict[row["Probability"]] #in Stage2 should be 1,0,3 or -2

            if zon_num ==5:
                    pzvFile2=  PPZV +pzx +"_v0_" + ".csv" 
                    pzvFile1=  PPZV_Stage1 +pzx +"_v0_" + ".csv" 
                    
                    pp_saver1(pzvFile2,pzvFile1,label,PZidq,pssgr)


            elif zon_num ==17:

                    pzvFile2=  PPZV+ pzx.replace("_z17","_z5")   +"_v32_.csv"
                    pzvFile1=  PPZV_Stage1 + pzx.replace("_z17","_z5")   +"_v32_.csv"
                    pp_saver1(pzvFile2, pzvFile1,label,PZidq,pssgr)
                ...
                for view in range(0,64,8):  #for a3daps
                    ...
                    if (my_vertices.loc[zon_num-1][vwx] ==1): 
                      try:
                        pzvFile2=  PPZV +  row["PZ"] +"_v" + str(view) + "_.csv"
                        pzvFile1=  PPZV_Stage1 +  row["PZ"] +"_v" + str(view) + "_.csv"
                        try:
                            img = np.genfromtxt(pzvFile2,delimiter='|')
                        except:
                            img = np.genfromtxt(pzvFile1,delimiter='|')
                        ...
                        if label ==[3,-2]:
                            ...
                            print("error, this shouldnt occur in Stage II")
                            #savHidden.append([PZidq,img])

                      except:

                        print("error_missing",pzvFile1)
    ...
    #after all minibatches have been compiled, save NewBatch PROB records
    
    
    #StageII files are too large to process in a single minibatch
    newSize = 1000
    NB_batches = int(len(savNewB)/newSize+.99) 
    for NewB_batch_num  in range(NB_batches): #batches number from zero
        
        #select a  batch of records as pzMinibatch
        u1= newSize * NewB_batch_num 
        u2 = min(newSize * (NewB_batch_num+1),len(savNewB))
        savNBn = np.array( savNewB[u1:u2],  dtype='object')
        np.save(PPDF + '/NewB_' + region+str(NewB_batch_num) ,savNBn)
...
def viewCombiner(Threat_Rgn)  :
    ...
    pzInvty = pd.read_csv(PATH1 + "/pz_invty5.csv",delimiter="|").set_index('PZidq')
    
    stage2 = pzInvty[pzInvty["Probability"]=="NewB"]["PZ"]
    ...
    with open(qfile,"w") as outFile:
        ...
        for k in range(len(pzvidE)):
          v1=stage2[pzvidE[k]].replace("_z","_Zone")
...
PPZV_Back = EnvX + r"/_np_zvwBack/" # EnvX + r"/_np_zvw2/"
PPZV_Stage1 = EnvX + r"/_np_zvw2_stage1OK/"  # EnvX + r"/_np_zvw2/" 
...
df1 = pd.read_csv(PATH1+ r"/passgr_data_invty2.csv",sep="|")
...
if control==4:  #revised
        ...
        if my_vertices.loc[4]["vw32"] ==1:
            ...
            try:
                ...
                if check==0:
                    ...
                    np.savetxt(PPZV_Back + each +  "_z5_v32_.csv", vw2, delimiter="|")
            except:
    ...
    print("\nRun Metrics\n")
    pzInvty = pd.read_csv("pz_invtyStage1_2_Combo.csv",delimiter="|") #header='infer',  index_col=None,
    #Stage1_2_Combo includes stage1 and incorporatesthe 100 previously hidden. It also has the 1388 StageII
    pzList = pzInvty["PZ"].tolist()
    pz2List = pzInvty [(pzInvty["Probability"]=="0") | (pzInvty["Probability"]=="1")  ]["PZ"].tolist()  

    print("override of the 59")
    for suffix in ["48"]:#"59"
        RunMetrix(PPZV_Stage1,pz2List,"spyd_"+suffix+".csv","dump"+suffix+".csv","R",suffix) #saves intermediate output
        ...
        print("\n compiling metrics for StageII passengers")
        pz2ListNB = pzInvty [ pzInvty["Probability"]=="NewB"]["PZ"].tolist()  #fix or pzInvty["Probability"]==1
        RunMetrix(PPZV,pz2ListNB,"NB"+suffix+".csv","dumpNB.csv","NB",suffix)
        X = MetrikTable [:,2:]
        predNB= logr.predict_proba(X)[:,1]	#Probability estimates.
        
        if suffix=="59":
            zoneFilter = ( (MetrikTable[:,2]<=5) | (MetrikTable[:,2]==9) |(MetrikTable[:,2]==17))
            predNB=predNB * zoneFilter
            
       
        pz3ListNB = np.array([pzInvty[pzInvty["PZ"]==x]["PZidq"].values[0] for x in pz2ListNB])
        newNB=     np.zeros((len(predNB),2)).astype(str) #changed from pred1. verify
        newNB[:,0] =   np.array(pz3ListNB)
        newNB[:,1] = np.char.mod('%1.8f', predNB) #changed from pred1 verify
        np.savetxt("predict"+suffix+"NB.csv", newNB, delimiter="|",fmt = "%s",header="pzvID|prediction" )  
...
#        PPDH=PPDF
    ...
    #implemented to allow intra-epoch shuffling
